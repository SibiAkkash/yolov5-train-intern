...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    clip_model, clip_transform = clip.load(name=model_filename, device=device, jit=False)
    clip_model.eval()
    
    img_encoder = gdet.create_box_encoder(clip_model, clip_transform, batch_size=1, device=device)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                trans_bboxes[:, 2:] -= trans_bboxes[:, :2] # or, trans_bboxes = xyxy2xywh(det[:, :4])
            ...
            detections = [
                ...
                for bbox, conf, class_num, feature in zip(bboxes, confs, class_nums, features)
            ]
            ...
            indices = preprocessing.non_max_suppression(boxes, class_nums, nms_max_overlap, scores)
