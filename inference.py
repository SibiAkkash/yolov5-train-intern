...
def run(
    ...
    model = torch.hub.load(
        ".", "custom", path="weights/frame-number-roi.pt", source="local"
    )
    ...
    dataset = LoadImages(
        path="../deep-text-recognition-benchmark/demo_2",
        img_size=640,
        stride=model.stride,
        auto=model.pt,
    )
...
def parse_opt():
    ...
    parser.add_argument(
        "--show-cycle-times", action="store_true", help="show cycle time barplot"
    )
