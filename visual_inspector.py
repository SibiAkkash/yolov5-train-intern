...
class VisualInspector:
    ...
    def _handle_cycle_start(self, frame_num: int):
        ...
        self.state["cycle_start_frame_time"] = get_time_from_frame(
            frame_num, self.stream_fps
        )
    ...
    def _handle_cycle_end(self, frame_num: int):
        ...
        self.state["cycle_end_frame_time"] = get_time_from_frame(
            frame_num, self.stream_fps
        )
        ...
        if self.num_cycles_seen > 1:
            ...
            with open("cycle_times_2.txt", "a") as f:
                global_cycle_time = (
                    self.state["cycle_end_sys_time"]
                    - self.state["cycle_start_sys_time"]
                )
    ...
    def get_object_closest_to_line(
        self, detections, object_id, line_y, check_top=True, buffer=30
    ):
        min_diff = float("inf")
        closest_obj_y = -1
    ...
    def _wait_for_horn_cross(
        self, detections: Dict[int, Tuple[int, int, int, int]], frame_num: int
    ):
    ...
    def _wait_for_inside_horn(
        ...
        inside_horn_y = self.get_object_within_lines(
            detections=detections, object_id=self.end_marker_object_id, check_top=False
        )
    ...
    def process_detections_2(
        ...
        if self.waiting_for_speedo_to_cross:
            return self._wait_for_speedo_cross(
                detections=detections, frame_num=frame_num
            )

        if self.waiting_to_see_inside_horn:
            return self._wait_for_inside_horn(
                detections=detections, frame_num=frame_num
            )
